...
\begin{document}
    ...
    \section{Painlev\'{e}--Gullstrand formalism for both branches}\label{sec:pg-framework}
        \subsection{Regular representation of the acoustic core}\label{subsec:pg-core}
            ...
            \end{equation}
            brings the metric~\eqref{eq:schwarzschild-metric} to the regular form:
            \begin{equation}
            ...
            \end{equation}
            for any $a > 0$, which reflects the hyperbolicity of the acoustic core and excludes coordinate degeneracies. The acoustic-horizon condition is equivalent to $\check{\eta}_{\check{t}\check{t}} = 0$ and reduces to~\cite{SemyannikovAV_GlobalAcousticGeometryOfMichelFlowInPainleveGullstrandCoordinates_2026}
            \begin{equation}
    ...
    \section{Transition between the degenerate branches: one-parameter model}\label{sec:transition-model}
        ...
        \subsection{Collective coordinate and effective Lagrangian}\label{subsec:collective-coordinate}
            ...
            \begin{figure}[htb]
                \centering
                \includegraphics[width=0.88\linewidth]{fig_inertia_localization.pdf}
        ...
        \subsection{Nature of the energy barrier}\label{subsec:barrier-origin}
            ...
            \begin{figure}[htb]
                \centering
                \includegraphics[width=0.70\linewidth]{fig_double_well_potential.pdf}
    ...
    \section{Discussion}\label{sec:discussion}
        ...
        \subsection{Connection with the surface gravity and quasinormal modes}\label{subsec:kappa-connection}
            ...
            \begin{figure}[htb]
                ...
                \caption{Dependence of the instanton action and the characteristic time scales on the asymptotic sound speed $a_{\infty}$. Left panel: the instanton action $\mathcal{S}_{\mathrm{inst}}$ computed from formulas~\eqref{eq:instanton-action-final} and~\eqref{eq:instanton-solution} using the values of $I$ and $\Delta E$ from Table~\ref{tab:instanton-parameters} (solid curves; markers --- tabulated values at $a_{\infty} = 0.05$ and $0.10$); dashed curves --- the approximate reference scaling $\propto a_{\infty}^{-2}$ normalized at the point $a_{\infty} = 0.1$. Right panel: the characteristic instanton time $\tau_{0} = \sqrt{I/(2\Delta E)}$ (solid curves; markers --- tabulated values) and the ringdown time $\tau_{\mathrm{QNM}} = \kappa^{-1}$ (gray dotted curves), where $\kappa$ is taken from Table~2 of the companion study~\cite{SemyannikovAV_AcousticGeometryAndSoundRayDynamicsInRelativisticSphericalAccretion_2026}. Over the entire range studied, $\tau_{0} \ll \tau_{\mathrm{QNM}}$, which confirms the scale separation $\tau_{0}^{-1} \gg \kappa$.}
